...
/** Pure NBT coordinate relocation used before a structure entity is instantiated. */
final class CreateElevatorEntityRelocation {
    ...
    static CompoundTag relocate(
        ...
        if (authoredEntityPosition == null) {
            return source;
        }
    ...
    private static void relocateAbsoluteY(
        CompoundTag contraption, String key, int verticalOffset
    ) {
        if (contraption.contains(key, Tag.TAG_ANY_NUMERIC)) {
            contraption.putInt(key, contraption.getInt(key) + verticalOffset);
        }
    }
    ...
    private static void relocateColumn(
        ...
        if (!contraption.contains("Column", Tag.TAG_COMPOUND)) {
            return;
        }
        CompoundTag column = contraption.getCompound("Column");
        BlockPos relative = new BlockPos(column.getInt("X"), 0, column.getInt("Z"));
        BlockPos transformed = transformRelative(relative, settings);
    ...
    private static Vec3 transformRelative(
        Vec3 vector, StructurePlaceSettings settings
    ) {
        double x = vector.x;
        double z = vector.z;
