...
- 작성된 메인 퀘스트의 진행 NPC 우선 표시와 다음 체육관 기본 목표 연동
- 현재 목표 방향으로 회전하는 흰색 화살촉과 노란 진행 꼬리 표시
- 센터·상점·체육관·카지노·동굴·숲·관문·던전용 소형 형태 아이콘
...
던전은 정의 파일의 목록이나 인스턴스 내부 좌표가 아니라, 런타임에 등록된 실제
진입 지점(건물·월드 구조물·동굴·숲 배치)을 표시한다. 같은 차원의 256블록 범위에서
보라색 문·계단 아이콘과 `던전: 이름`으로 표시되며 `입구와 관문` 설정을 따른다.
건물·동굴 마커와 겹치면 던전 표시를 우선한다. 마커 프로토콜은 v3이므로 서버와
클라이언트의 Pokefinder 모듈을 함께 갱신하고 World Bootstrap도 새 빌드를 적용해야 한다.
