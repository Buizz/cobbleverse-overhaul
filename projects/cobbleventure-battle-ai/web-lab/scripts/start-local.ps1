...
function Test-LocalPort {
    try {
        ...
        return $response.Content -like "*Cobbleventure Battle Lab*"
    }
    catch {
        return $false
    }
...
if (Test-Path -LiteralPath $pidFile) {
    ...
    if ($savedProcess -and (Test-LocalPort)) {
        Write-Host "Cobbleventure Battle Lab is already running: $localUrl"
        if (-not $NoBrowser) {
            Open-LocalPage
        }
        exit 0
...
if (Test-PortOccupied) {
    Write-Error "Port 3000 is already in use by another process. Cobbleventure Battle Lab only uses http://localhost:3000; stop the occupying process and run this script again."
    exit 1
}
...
Write-Host "Cobbleventure Battle Lab started: $localUrl"
if (-not $NoBrowser) {
    Open-LocalPage
}
