...
const BATTLE_ENGINES = new Set(["showdown", "cobbleventure"]);
...
export function createBattleScenario(raw, trainers, itemResolver = null) {
  ...
  if (!BATTLE_ENGINES.has(battleEngine)) {
    issues.push(
      issue(
        "battleEngine",
        "unsupported",
        "배틀 엔진은 showdown 또는 cobbleventure여야 합니다.",
      ),
    );
  }
  ...
  const gimmickRules =
    battleEngine === "cobbleventure" ? "all" : requestedGimmickRules;
  if (!GIMMICK_RULESETS.has(gimmickRules)) {
    ...
    issues.push(
      issue(
        ...
        "전체 기믹 규칙은 Cobbleventure 자체 엔진에서만 사용할 수 있습니다.",
      ),
    );
  }
  ...
  } else if (battleEngine === "cobbleventure" && !format.supportsCobbleventure) {
    issues.push(
      issue(
        "battleType",
