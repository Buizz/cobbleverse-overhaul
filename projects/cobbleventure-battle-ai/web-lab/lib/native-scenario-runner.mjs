...
import { runSimpleBattle } from "./cobbleventure-battle-engine.mjs";
...
export function runNativeScenarioBattle(scenario, options = {}) {
  if ((scenario.battleType ?? "single") !== "single") {
    throw new Error("Cobbleventure 자체 엔진은 현재 싱글 배틀만 지원합니다.");
  }
    ...
    warnings: [
      {
        ...
        message:
          "자체 엔진은 현재 포켓몬·기술 원본 데이터만 Showdown 카탈로그에서 읽으며, 전투 판정은 Cobbleventure 엔진이 수행합니다.",
      },
    ],
  };
