...
import {
  ...
  resolveSimpleTurn,
} from "./cobbleventure-battle-engine.mjs";
import {
...
function reproductionLog(session) {
  return {
    schema: "cobbleventure-native-pve-reproduction",
    version: 1,
    ...
    warnings: [
      {
        ...
        message:
          "Cobbleventure 자체 엔진 PvE는 상태 전이 기반 개발 버전입니다. 효과 카탈로그에 없는 기술은 전투 로그에 명시됩니다.",
      },
    ],
    error: null,
...
export function startNativeInteractiveBattle(scenario) {
  ...
  if ((scenario.battleType ?? "single") !== "single") {
    throw new Error("Cobbleventure 자체 엔진 PvE는 현재 싱글 배틀만 지원합니다.");
  }
...
export function exportNativeInteractiveBattleSave(sessionId) {
  ...
  return {
    schema: "cobbleventure-pve-battle-save",
    version: 1,
    battleEngine: "cobbleventure",
    scenario: structuredClone(session.scenario),
    turn: session.state.turn,
...
export function resumeNativeInteractiveBattle(save) {
  if (
    save?.schema !== "cobbleventure-pve-battle-save" ||
    save?.version !== 1 ||
    save?.battleEngine !== "cobbleventure" ||
    !save?.scenario ||
    !save?.payload?.checkpoint?.state
  ) {
    throw new Error("올바른 Cobbleventure PvE 전투 저장 데이터가 아닙니다.");
  }
  removeExpiredSessions();
  const session = {
    id: `native-${randomUUID()}`,
