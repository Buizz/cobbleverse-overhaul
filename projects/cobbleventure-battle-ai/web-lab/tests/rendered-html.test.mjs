...
test("server-renders the Cobbleventure battle lab shell", async () => {
  ...
  assert.match(html, /<title>Cobbleventure Battle Lab(?: · Cobbleventure)?<\/title>/i);
...
test("returns exact native turn HP snapshots through the battle API", async () => {
  ...
  assert.equal(response.status, 201, JSON.stringify(body));
  assert.equal(body.ok, true);
  assert.equal(body.battle.engine.id, "cobbleventure-simple");
