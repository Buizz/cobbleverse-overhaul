...
이 문서는 전투 AI 고도화의 첫 구현 단위를 추적한다. 목표는 AI를 더 똑똑하게 만들기 전에, Showdown 기준 엔진과 Cobbleventure 자체 엔진이 같은 관측 모델과 행동 후보 평가 로그를 AI에 제공하게 만드는 것이다.
...
| 영역 | 상태 |
|------|------|
| 전투 엔진 | PvE/EvE 실험이 가능한 기반이 있으며, Showdown 기준 엔진과 Cobbleventure 자체 엔진이 공존한다. |
...
- [x] Cobbleventure 자체 엔진 상태를 `AiBattleObservation`으로 변환
...
type AiBattleType = {
  ...
  ruleset?: string[];
  engine: "showdown" | "cobbleventure-simple" | "cobblemon";
};
```
...
- [x] Cobbleventure 자체 엔진 상태에서 공통 관측으로 변환하는 함수를 만든다.
...
- Showdown PvE/EvE와 Cobbleventure 자체 엔진 PvE/EvE가 공통 관측 모델을 통해 AI를 호출한다.
