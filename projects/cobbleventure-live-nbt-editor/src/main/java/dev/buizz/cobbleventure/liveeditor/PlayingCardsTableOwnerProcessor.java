...
/** Supplies the ownership data that Playing Cards normally adds on player placement. */
final class PlayingCardsTableOwnerProcessor extends BlockIgnoreProcessor {
    ...
    static final ResourceLocation POKER_TABLE = ResourceLocation.fromNamespaceAndPath(
    ...
    static CompoundTag withOwner(CompoundTag source) {
        if (source != null && source.contains("OwnerID", Tag.TAG_INT_ARRAY)) return source;
