...
파란색 텍스처는 참고 이미지의 평균색 `#4576C8`을 유지하되, 확대된 타일 무늬 없이
`128×128`의 각 픽셀에 독립적인 미세 노이즈를 적용합니다. 파랑은 채널별 변화 폭을
달리해 낮은 대비 안에서도 재질감이 보이게 하며, 모든 대상 텍스처가 1픽셀 단위 패턴을 사용합니다.

## 요철 타일 작업 목록

다음 블록은 정육면체 텍스처의 일부를 돌출시키거나 안쪽으로 홈 처리한 타일입니다.

- `underground_blue_band` — 적용 완료: 하단 4픽셀을 전면으로 1픽셀 돌출
- `underground_cracked_wall` — 적용 완료: 녹색 꺾임 몰딩을 전면으로 1픽셀 돌출
- `underground_olive_band` — 지하통로 올리브 나무 바닥. 어두운 나무결 2줄을 여섯 면 모두 1픽셀 안쪽으로 홈 처리하며 설치 및 NBT 구조물 변환 시 항상 북쪽 기준으로 고정
- `house_mint_band_wall` — 적용 완료: 이미지 10~13행의 짙은 띠를 전면으로 1픽셀 돌출
- `house_blue_band_wall` — 적용 완료: 이미지 10~13행의 짙은 띠를 전면으로 1픽셀 돌출
- `house_beige_panel_wall` — 주택 베이지 나무 바닥. 어두운 나무결 4줄을 여섯 면 모두 1픽셀 안쪽으로 홈 처리하며 설치 및 NBT 구조물 변환 시 항상 북쪽 기준으로 고정
- `casino_coral_band` — 적용 완료: 하단 4픽셀을 전면으로 1픽셀 돌출
- `casino_sky_chevron_wall` — 적용 완료: 밝은 V자 몰딩을 전면으로 1픽셀 돌출

## 입체 설비 타일

- `rocket_base_olive_vent` — 요철 제거 완료: 텍스처만 사용하는 평평한 정육면체
- `rocket_base_yellow_light_panel` — 요철 제거 완료: 텍스처만 사용하는 평평한 정육면체
...
| 로켓단 기계 2 | 가로 1 × 높이 2 |
| 로켓단 기계 3 | 가로 2 × 높이 3 |
| 오박사 연구소 기계 | 바닥 2 × 2 × 높이 2, 설치 블록은 모서리 기준점, 중앙은 곧은 원통 |
