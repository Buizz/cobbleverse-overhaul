...
/** Reads the same global menu tokens used by the player menu and bag. */
final class CasinoMenuTheme {
    ...
    private CasinoMenuTheme(JsonObject root) {
    ...
    static CasinoMenuTheme load(Minecraft minecraft) {
        if (minecraft != null) {
            try {
                ...
                if (resource.isPresent()) try (Reader reader = resource.get().openAsReader()) {
                    return new CasinoMenuTheme(JsonParser.parseReader(reader).getAsJsonObject());
                }
            } catch (Exception ignored) {}
        }
        return new CasinoMenuTheme(new JsonObject());
    }

    int mutedText() {
        return CasinoThemedPanel.withOpacity(textColor, .65F);
    }
