...
던전 조각은 논리 형태와 테마 외형을 분리한다. `standard_16` 키트는 X/Z 16블록을
최소 격자 단위로 사용한다. 통로 조각은 기본적으로 `16×8×16`이며, 공동은
`16×8×32`, `32×8×32`처럼 격자의 배수 크기를 점유할 수 있다. 같은 형태의 모든
스킨은 크기, 커넥터 ID·좌표·방향, 마커 슬롯을 동일하게 유지하므로 같은 배치
계획에서 스킨별 `piece_id`만 대응하는 형태로 교체할 수 있다.
