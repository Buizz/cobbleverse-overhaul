...
class ContentManagerTests(unittest.TestCase):
    ...
    def test_dungeon_web_defaults_piece_content_to_marker_placement(self) -> None:
        ...
        self.assertIn('NPC 배치·공간', script)
        self.assertIn('${plan.npcCapacity.actorDemand}명 배치', script)
    ...
    def test_dungeon_uses_one_spatial_algorithm_and_selected_chambers(self) -> None:
        ...
        self.assertIn("const separationFromAssigned =", script)
        self.assertIn("(roomOccupancy.get(a[0]) || 0) - (roomOccupancy.get(b[0]) || 0)", script)
    ...
    def test_dungeon_layout_uses_simple_complexity_presets_and_advanced_details(self) -> None:
        ...
        self.assertIn('name="floorCountMin" type="number"', script)
        self.assertIn('name="floorCountMax" type="number"', script)
        self.assertIn('|| "custom"', script)
        self.assertIn("floorRange[1] - floorRange[0] + 1", script)
        self.assertIn(".dungeon-complexity-control", styles)
        self.assertIn(".dungeon-floor-range", styles)
