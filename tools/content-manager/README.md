...
- 던전 관리 화면에서 전역 옵션과 내부 조우·치료소·전리품·관문을 편집하고, 게시형 계획의 NBT 조각 배치·회전·커넥터 연결·저장 및 런타임 계획의 시드별 2D 평면도 확인
...
같은 화면의 **커스텀 가챠 기계** 영역은 콘텐츠 프로젝트의
`content/catalogs/gacha-machines.json`을 편집한다. 각 프로필은 본체·강조 블록 외형,
화폐, 희귀도별 아이템/포켓몬 보상과 소프트·확정·선택 천장을 가진다. 저장 후
`build.bat mod-casino`를 실행하면 `cobbleventure-casino` 애드온 JAR에 반영된다.
