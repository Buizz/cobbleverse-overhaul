...
const defaultWorldEntranceStructures = {
  ...
  forest: "cobbleventure:forest_gate/forest_gate"
};
const biomeChoices = {
...
let openMusicCombobox = null;

function scheduleMusicComboboxRefresh() {
  if (musicComboboxRefreshPending) return;
  musicComboboxRefreshPending = true;
  queueMicrotask(() => {
    musicComboboxRefreshPending = false;
    const selects = new Set($$("option[data-music-option]").map((option) => option.closest("select")).filter(Boolean));
    selects.forEach(enhanceMusicSelect);
  });
}

function enhanceMusicSelect(select) {
  if (select.musicCombobox) {
    select.musicCombobox.refresh();
    return;
  }

  const wrapper = document.createElement("div");
  wrapper.className = "music-combobox";
  ...
  toggle.type = "button";
  toggle.className = "music-combobox-toggle";
  toggle.setAttribute("aria-label", `${label} 목록 열기`);
  toggle.textContent = "⌄";

  const list = document.createElement("div");
  list.className = "music-combobox-list";
  list.setAttribute("role", "listbox");
  list.hidden = true;

  select.before(wrapper);
  select.classList.add("music-native-select");
  ...
  const setOpen = (open) => {
    if (open && openMusicCombobox && openMusicCombobox.wrapper !== wrapper) openMusicCombobox.close();
    wrapper.classList.toggle("is-open", open);
    list.hidden = !open;
    input.setAttribute("aria-expanded", String(open));
    if (open) openMusicCombobox = { wrapper, close: () => setOpen(false) };
    else {
      activeIndex = -1;
      if (openMusicCombobox?.wrapper === wrapper) openMusicCombobox = null;
    }
  };

  const renderMatches = (query = "") => {
    const needle = query.trim().toLocaleLowerCase();
    matches = options().filter((option) => !needle || option.label.toLocaleLowerCase().includes(needle));
    if (activeIndex >= matches.length) activeIndex = matches.length - 1;
    list.innerHTML = matches.length
      ? matches.map((option, index) => `<button type="button" role="option" data-music-combobox-value="${escapeHtml(option.value)}" class="${option.value === select.value ? "is-selected" : ""} ${index === activeIndex ? "is-active" : ""}" aria-selected="${option.value === select.value}">${escapeHtml(option.label)}</button>`).join("")
      : '<span class="music-combobox-empty">일치하는 음원이 없습니다.</span>';
  };
  ...
  const commitExactOrRestore = () => {
    const typed = input.value.trim().toLocaleLowerCase();
    const exact = options().find((option) => option.label.toLocaleLowerCase() === typed);
    if (!commit(exact)) input.value = select.selectedOptions[0]?.textContent?.trim() || "";
  };

  const open = () => {
    renderMatches(input.value === (select.selectedOptions[0]?.textContent?.trim() || "") ? "" : input.value);
    setOpen(true);
  };
  ...
  input.addEventListener("change", commitExactOrRestore);
  input.addEventListener("keydown", (event) => {
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      event.preventDefault();
      commit(matches[activeIndex] || (matches.length === 1 ? matches[0] : null));
  ...
  toggle.addEventListener("click", () => {
    if (list.hidden) { input.focus(); open(); }
    else setOpen(false);
  });
  list.addEventListener("mousedown", (event) => event.preventDefault());
  list.addEventListener("click", (event) => {
    const optionButton = event.target.closest("[data-music-combobox-value]");
    if (!optionButton) return;
    commit(options().find((option) => option.value === optionButton.dataset.musicComboboxValue));
  });
  select.addEventListener("change", () => {
    input.value = select.selectedOptions[0]?.textContent?.trim() || "";
  });
  select.musicCombobox = {
    refresh() {
      input.disabled = select.disabled;
      toggle.disabled = select.disabled;
      input.value = select.selectedOptions[0]?.textContent?.trim() || "";
      if (!list.hidden) renderMatches(input.value);
    }
  };
  select.musicCombobox.refresh();
}
...
function musicOptions(selected = "", inheritContext = "") {
  ...
  const first = inheritContext
    ? `<option value="" data-music-option>상속 · ${escapeHtml(musicTrackLabel(inherited))}</option>`
    : "";
  const markup = first + (state.musicCatalog.tracks || []).map((track) =>
    `<option value="${escapeHtml(track.id)}" data-music-option ${track.id === selected ? "selected" : ""}>${escapeHtml(musicTrackLabel(track.id))}</option>`
  ).join("");
  scheduleMusicComboboxRefresh();
  return markup;
}

document.addEventListener("pointerdown", (event) => {
  if (openMusicCombobox && !openMusicCombobox.wrapper.contains(event.target)) openMusicCombobox.close();
});
...
async function refreshMusicLibrary() {
  ...
  renderMusicSettings();
  const library = result.data.local_library || {};
  toast(library.added || library.removed
    ? `폴더 기준으로 동기화했습니다. 추가 ${library.added || 0}곡 · 제거 ${library.removed || 0}곡`
    : "폴더의 OGG 음원 목록이 최신 상태입니다.");
}
...
function validatePlayerConditions(conditions) {
  return (conditions || []).map((condition, index) => {
    ...
    if (condition.type === "item") {
      ...
      return { type: "item", item: condition.item, count: Math.max(1, Math.floor(Number(condition.count) || 1)), ...(condition.negate ? { negate: true } : {}) };
    }
...
function initializeGateConditionEditor(editor) {
  ...
  editor.addEventListener("click", (event) => {
    ...
    renderGateConditionEditor(editor);
    if (editor.closest("#tile-inspector-form")) applyTilePlacement();
  });
...
const routeEncounterMethods = {
  land: { label: "일반 조우", help: "길을 걸을 때 발생하는 일반 야생 조우입니다." },
  surf: { label: "파도타기", help: "물 위를 이동할 때 사용하는 전용 출현 풀입니다." },
  old_rod: { label: "낡은낚싯대", help: "낡은낚싯대로 낚을 때 사용하는 전용 출현 풀입니다." },
  good_rod: { label: "좋은낚싯대", help: "좋은낚싯대로 낚을 때 사용하는 전용 출현 풀입니다." },
  super_rod: { label: "대단한낚싯대", help: "대단한낚싯대로 낚을 때 사용하는 전용 출현 풀입니다." },
...
function ensureRouteEncounterPool(root, method) {
  ...
  root.encounter_pools ||= {};
  root.encounter_pools[method] ||= newRouteEncounterPool();
  const pool = root.encounter_pools[method];
  pool.enabled = pool.enabled === true;
  pool.inherit_biome = pool.inherit_biome === true;
  pool.excluded_species ||= [];
  pool.additions ||= [];
...
function encounterPokemonIconMarkup(settings, species, byId, defaultRange = null) {
  const entry = byId.get(species); const override = pokemonLevelOverride(settings, species);
  const minimum = override?.min_level ?? defaultRange?.min ?? settings.minimum_level; const maximum = override?.max_level ?? defaultRange?.max ?? settings.maximum_level;
...
function renderEncounterSummary(target) {
  ...
  $(`[data-encounter-pokemon-count="${target}"]`).textContent = settings.enabled ? `${species.length}종` : `비활성 · ${species.length}종 후보`;
  $(`[data-encounter-pokemon-description="${target}"]`).textContent = settings.enabled
    ...
    : `주변 인카운터 사용 안 함 · 활성화하면 ${species.length}종이 출현 후보가 됩니다.`;
...
function handlePokemonCardEditAction(event, target, render) {
  ...
  const open = event.target.closest(`[data-${target}-pokemon-card-open]`);
  if (!edit && !done && !open) return false;
  state[`${target}PokemonEditingCard`] = done ? null : (edit?.dataset[`${target}PokemonCardEdit`] || open?.dataset[`${target}PokemonCardOpen`]);
...
function pokemonCardLevelFields(settings, species, target, defaultRange) {
  ...
  const minimum = override?.min_level ?? defaultRange.min;
  const maximum = override?.max_level ?? defaultRange.max;
  return `<div class="pokemon-card-level-fields"><span>레벨</span><input type="number" min="1" max="100" value="${minimum}" title="최소 레벨" aria-label="최소 레벨" data-${target}-card-level-min="${escapeHtml(species)}"><i aria-hidden="true">–</i><input type="number" min="1" max="100" value="${maximum}" title="최대 레벨" aria-label="최대 레벨" data-${target}-card-level-max="${escapeHtml(species)}"><button type="button" class="pokemon-card-icon-action" data-${target}-card-level-reset="${escapeHtml(species)}" title="기본 레벨로 되돌리기" aria-label="기본 레벨로 되돌리기" ${override ? "" : "disabled"}>↺</button></div>`;
}
...
function pokemonCardEditHeading(name, number, target, kind, species, extraAction = "") {
  return `<header class="pokemon-card-edit-heading"><div><strong>${escapeHtml(name)}</strong>${number ? `<small>No.${String(number).padStart(4, "0")}</small>` : ""}</div><span class="pokemon-card-heading-actions">${extraAction}${pokemonCardEditButton(target, kind, species, true)}</span></header>`;
}
function pokemonCardIdentityMarkup(name, dex) {
...
function routePresetPokemonLevelRange(preset, route) {
  const scaling = preset?.level_scaling || {};
  if (scaling.mode === "fixed") return { min: Number(scaling.minimum_level || 1), max: Number(scaling.maximum_level || 100) };
  const range = route ? routeDefaultPokemonLevelRange(route) : { min: 1, max: 100 };
  if (scaling.mode !== "offset") return range;
  const offset = Number(scaling.offset || 0);
  return { min: Math.max(1, Math.min(100, range.min + offset)), max: Math.max(1, Math.min(100, range.max + offset)) };
}
...
function addSelectedEncounterPokemon() {
  ...
  settings.additions.push(...selected.map((entry) => ({ species: entry.id, min_level: settings.minimum_level, max_level: settings.maximum_level, spawn_as_evolved: false, weight: 1 })));
...
function routePokemonSettings(method = state.routePokemonMethod) {
  const document = routePokemonDocument();
  if (!document) return null;
  return ensureRouteEncounterPool(ensureRoutePokemonSettings(document), method);
}
...
function routePokemonCardCopy(entry, includeName = true) {
  ...
  return `<div class="route-pokemon-card-copy">${includeName ? `<b class="route-pokemon-name">${escapeHtml(pokemonMapEntryName(entry))}</b>` : ""}<small>No.${String(entry.dex_number).padStart(4, "0")}</small><span class="route-pokemon-type-badges">${typeBadges}</span></div>`;
}
...
function renderRoutePokemonDialog() {
  ...
  $$("[data-route-encounter-method]").forEach((button) => {
    const pool = button.dataset.routeEncounterMethod === "land" ? rootSettings : rootSettings.encounter_pools?.[button.dataset.routeEncounterMethod];
    const count = pool?.additions?.length || 0;
    button.classList.toggle("is-active", button.dataset.routeEncounterMethod === state.routePokemonMethod);
    button.innerHTML = `${escapeHtml(routeEncounterMethods[button.dataset.routeEncounterMethod].label)}<small>${button.dataset.routeEncounterMethod === "land" || pool?.enabled ? `${count}종` : "사용 안 함"}</small>`;
  });
  const methodSettings = $(".route-encounter-method-settings");
  methodSettings.classList.toggle("is-land", state.routePokemonMethod === "land");
  $("#route-encounter-method-enabled").checked = state.routePokemonMethod === "land" || settings.enabled === true;
  $("#route-encounter-method-enabled").disabled = state.routePokemonMethod === "land";
  $("#route-encounter-trigger-chance").value = Math.round((state.routePokemonMethod === "land" ? 1 : settings.trigger_chance) * 100);
  $("#route-encounter-trigger-chance").disabled = state.routePokemonMethod === "land";
  $("#route-encounter-method-help").textContent = method.help;
...
function addSelectedRoutePokemon() {
  ...
  if (!selected.length) return;
  settings.additions.push(...selected.map((entry) => ({ species: entry.id, min_level: 1, max_level: 100, spawn_as_evolved: false, weight: 1 })));
...
function renderWorldPokemonPanel() {
  ...
  const layout = state.worldLayout;
  const configuredGenerations = [...new Set((layout?.pokemon_generations || data.pokemon_generations || [state.selectedGeneration]).map(Number).filter((value) => value >= 1 && value <= 9))].sort((a, b) => a - b);
  const pokemonGenerations = configuredGenerations.length ? configuredGenerations : [state.selectedGeneration];
  if (layout) layout.pokemon_generations = pokemonGenerations;
  $("#world-pokemon-generation-toggles").innerHTML = Array.from({ length: 9 }, (_, index) => index + 1).map((generation) => {
    const active = pokemonGenerations.includes(generation);
    return `<button type="button" class="${active ? "is-active" : ""}" data-world-pokemon-generation="${generation}" aria-pressed="${active}">${generation}세대</button>`;
  }).join("");
  ...
  $("#pokemon-map-generation").textContent = `${pokemonGenerations.join("·")}세대`;
  ...
  if (state.pokemonMapTab === "unavailable") {
    ...
    $("#pokemon-map-location").innerHTML = `<b>${pokemonGenerations.join("·")}세대 대상 미출현 포켓몬</b><span>선택한 세대 중 모든 출현 지역과 길의 직접 추가 목록 어디에도 포함되지 않은 포켓몬입니다.</span>`;
  } else {
    entries = data.available_pokemon || [];
    $("#pokemon-map-location").innerHTML = `<b>${pokemonGenerations.join("·")}세대 월드 출현 포켓몬</b><span>선택 세대의 자동 출현과 직접 추가 예외를 합친 목록입니다.</span>${state.worldDirty ? "<small>세대 변경은 월드를 저장한 뒤 목록에 반영됩니다.</small>" : ""}`;
  }
...
function renderList(category) {
  const allItems = state[category];
  const items = category === "trainers" && state.npcFilter !== "all"
  ...
  if (category === "trainers") {
    ...
    $$('[data-npc-filter]').forEach((button) => button.classList.toggle("is-active", button.dataset.npcFilter === state.npcFilter));
    Object.entries(counts).forEach(([key, value]) => { const output = $(`[data-npc-filter-count="${key}"]`); if (output) output.textContent = value; });
  }
...
function trainerPopulationConfig(scope) {
  if (scope === "route") {
    ...
    const value = state.routePreset.automatic_npc_placement;
    return { value, countKey: "count" };
  }
  if (scope === "cave") {
    state.cave.trainer_settings ||= { enabled: false, max_active: 0, class_pool: [], placements: [] };
    return { value: state.cave.trainer_settings, countKey: "max_active" };
  }
  if (scope === "forest") {
    state.forest.trainer_settings ||= { enabled: false, max_active: 0 };
    return { value: state.forest.trainer_settings, countKey: "max_active" };
  }
...
function updateSharedTrainerPopulation(scope) {
  ...
  if (!container) return;
  const { value, countKey } = trainerPopulationConfig(scope);
  value.enabled = container.querySelector('[data-trainer-population-field="enabled"]').checked;
...
function automaticNpcCandidates({ classification, level = 5, biomes = [], target = "town" }) {
  const biomeSet = new Set((biomes || []).filter(Boolean));
  return state.trainers
    .filter((npc) => (npc.classification || (npc.battle_type ? "trainer" : "ambient")) === classification)
    .filter((npc) => {
      if (target === "route") return npc.automatic_route_placement;
      if (target === "town") return npc.automatic_town_placement;
      return classification === "trainer";
    })
    .map((npc) => {
      const preferred = npc.preferred_biomes || [];
      const biomeMatch = preferred.some((biome) => biomeSet.has(biome));
      const biomeScore = biomeMatch ? 0 : preferred.length ? 100 : 20;
      const levelScore = Number.isInteger(npc.expected_level) ? Math.abs(npc.expected_level - level) : 15;
      return { ...npc, matchScore: biomeScore + levelScore, biomeMatch };
    })
    .sort((left, right) => left.matchScore - right.matchScore || String(left.id).localeCompare(String(right.id)));
}

function resolvedTrainerCandidates(population, { level, biomes, target }) {
  const directIds = population?.direct_trainers || [];
  const direct = directIds.map((id) => state.trainers.find((trainer) => trainer.id === id)).filter(Boolean);
  const automatic = population?.use_biome_defaults === false ? [] : automaticNpcCandidates({ classification: "trainer", level, biomes, target });
  return [...direct, ...automatic.filter((trainer) => !directIds.includes(trainer.id))];
}
...
function renderRouteNpcList() {
  ...
  list.innerHTML = placements.length ? placements.map((placement, index) => `
    <article class="route-npc-row" data-route-npc="${index}">
      ...
      <label><span>개별 조우 정책</span><select data-route-npc-field="trigger_override"><option value="proximity">승리 전 자동 도전 → 승리 후 말 걸기 (지역 기본)</option><option value="interact">항상 플레이어가 말 걸기</option><option value="preset">NPC 행동 프리셋 사용</option></select></label>
...
function renderRoutePreset() {
  ...
  renderSharedTrainerPopulation("route");
...
function updateCaveFromForm() {
  ...
  if (form.elements.musicTrack.value) state.cave.music_track = form.elements.musicTrack.value;
  else delete state.cave.music_track;
...
function renderForest() {
  ...
  form.elements.musicTrack.innerHTML = musicOptions(document.music_track || "", "forest");
  setFormValue(form, "musicTrack", document.music_track || "");
...
function updateForestFromForm() {
  ...
  if (form.elements.musicTrack.value) document.music_track = form.elements.musicTrack.value;
  else delete document.music_track;
  ...
  updateSharedTrainerPopulation("forest");
...
function renderTrainer() {
  ...
  const placementProfile = document.placement_profile || {
    classification: attachedBattle ? "trainer" : "ambient",
    preferred_biomes: [], automatic_town_placement: false, automatic_route_placement: false
  };
  setFormValue(form, "npcClassification", placementProfile.classification || (attachedBattle ? "trainer" : "ambient"));
  setFormValue(form, "npcExpectedLevel", placementProfile.expected_level ?? "");
  setFormValue(form, "npcPreferredBiomes", (placementProfile.preferred_biomes || []).join(", "));
  setFormValue(form, "npcAutoTown", Boolean(placementProfile.automatic_town_placement));
  setFormValue(form, "npcAutoRoute", Boolean(placementProfile.automatic_route_placement));
  form.elements.npcAutoRoute.disabled = placementProfile.classification === "ambient";
  const categoryNames = {
...
function updateTrainerFromForm() {
  ...
  const classification = form.elements.npcClassification.value;
  const expectedLevel = Number.parseInt(form.elements.npcExpectedLevel.value, 10);
  state.trainer.placement_profile = {
    classification,
    ...(Number.isInteger(expectedLevel) ? { expected_level: Math.max(1, Math.min(100, expectedLevel)) } : {}),
    preferred_biomes: [...new Set(form.elements.npcPreferredBiomes.value.split(",").map((value) => value.trim()).filter(Boolean))],
    automatic_town_placement: form.elements.npcAutoTown.checked,
    automatic_route_placement: classification === "trainer" && form.elements.npcAutoRoute.checked,
  };
  form.elements.npcAutoRoute.disabled = classification === "ambient";
  if (classification === "ambient") form.elements.npcAutoRoute.checked = false;
...
function renderEventDesignMode() {
  ...
  $("#event-trigger-type").value = trigger.type || "interact";
...
function changeEventDesignMode() {
  ...
  if (mode === "easy_npc_events") {
    ...
    state.trainer.events = [{ id: "primary", trigger, commands: [] }];
    applyEventScriptPreset();
    toast("직접 이벤트 설계 모드로 전환했습니다. 이후 프리셋 자동 갱신은 적용되지 않습니다.");
    return;
  }
  const existing = state.trainer.event_design?.preset;
  state.trainer.event_design = { mode: "preset", preset: existing || {
    type: "simple",
    initial_trigger: { type: "interact", range: 4 },
    first_text: { ko_kr: $("#event-preset-first-text").value.trim() || "안녕하세요!" },
  } };
  renderEventScript();
  applyNormalizedEventPreset();
}
...
function clearKeyChoices() {
  ...
  const presetClearKey = state.trainer?.event_design?.preset?.clear_key;
  if (presetClearKey) choices.push([presetClearKey, "현재 행동 프리셋에서 사용 중"]);
  for (const event of state.trainer?.events || []) {
...
function localizedPresetText(value, fallback = "") {
  return value && typeof value === "object" ? (value.ko_kr || value.en_us || fallback) : String(value || fallback);
}

function eventPresetIsBattle(type = $("#event-preset-type").value) {
  return ["battle", "gym", "elite", "champion"].includes(type);
}

function renderNormalizedEventPreset() {
  const design = state.trainer?.event_design;
  if (design?.mode !== "preset") return;
  const preset = design.preset || {};
  $("#event-preset-type").value = preset.type || "simple";
  ...
  $("#event-preset-win-item").value = preset.win_item || "";
  ...
  $("#event-preset-clear-key").dataset.currentValue = clearKey;
  $("#event-preset-clear-key-custom").value = $("#event-preset-clear-key").value === "__custom__" ? clearKey : "";
  updatePresetClearKeyMode();
}

function applyNormalizedEventPreset() {
  const type = $("#event-preset-type").value;
  const previous = state.trainer.event_design?.preset || {};
  const preset = {
    type,
    initial_trigger: {
      type: $("#event-trigger-type").value,
      range: Math.max(.5, Number($("#event-trigger-range").value) || 4),
    },
    first_text: { ko_kr: $("#event-preset-first-text").value.trim() || "안녕하세요!" },
  };
  if (preset.initial_trigger.type === "proximity") {
    preset.initial_trigger.warning_offset = Math.max(0, Number($("#event-warning-offset").value) || 0);
    preset.initial_trigger.indicator = "trainer_nearby";
  }
  ...
  if (eventPresetIsBattle(type)) {
    ...
    Object.assign(preset, {
      ...
      win_text: { ko_kr: $("#event-preset-win-text").value.trim() || "좋은 승부였어!" },
    ...
    if (type === "gym") preset.badge = $("#event-preset-badge").value;
    const winItem = $("#event-preset-win-item").value.trim();
    if (winItem) {
      preset.win_item = winItem;
      preset.win_item_count = Math.max(1, Number($("#event-preset-win-item-count").value) || 1);
    }
    ...
    state.trainer.npc.battle_rewards = { money: {
      enabled: winMoney > 0, mode: "fixed", amount: winMoney,
  ...
  state.trainer.event_design = { mode: "preset", preset };
  delete state.trainer.events;
...
function updateEventTrigger() {
  if (state.trainer?.event_design?.mode === "preset") {
    const preset = state.trainer.event_design.preset;
    const type = $("#event-trigger-type").value;
    preset.initial_trigger = { type, range: Math.max(.5, Number($("#event-trigger-range").value) || 4) };
    if (type === "proximity") {
      preset.initial_trigger.warning_offset = Math.max(0, Number($("#event-warning-offset").value) || 0);
      preset.initial_trigger.indicator = "trainer_nearby";
    }
...
function gymLeagueOptions(selected = "") {
  ...
  return '<option value="">나중에 지정</option>' + leaders.map((entry) => `<option value="${escapeHtml(entry.id)}"${entry.id === selected ? " selected" : ""}>${escapeHtml(entry.display_name?.ko_kr || entry.id)} · 도전 Lv.${entry.level_cap}</option>`).join("");
}
...
function manualTownDecorations() {
  const value = state.settlement?.structure_profile?.decoration_placements;
  return Array.isArray(value) ? value : null;
}

function setManualTownDecorations(value) {
  state.settlement.structure_profile ||= {};
  state.settlement.structure_profile.decoration_placements = value;
}

function villageCanvasWorldPosition(canvas, event) {
  const bounds = canvas.getBoundingClientRect();
...
function commitTownDecorationEdit() {
  $("#settlement-json").value = JSON.stringify(state.settlement, null, 2);
  renderVillageGenerationTest();
}

function renderTownDecorationControls() {
  const editor = state.villageDecorationEditor;
  const placements = manualTownDecorations();
  const selected = placements?.[editor.selected];
  $("#town-decoration-convert").disabled = placements !== null;
  $("#town-decoration-delete").disabled = !selected;
  $("#town-decoration-rotation").disabled = !selected;
  if (selected) {
    $("#town-decoration-type").value = selected.type;
    $("#town-decoration-rotation").value = selected.rotation || "none";
  }
  $("#town-decoration-status").textContent = placements === null
    ? "자동 배치 미리보기 · 가져온 뒤 직접 편집"
    : editor.tool === "place"
      ? `지도에서 ${$("#town-decoration-type").selectedOptions[0].textContent} 위치를 선택하세요`
      : `수동 장식 ${placements.length}개${selected ? ` · X ${selected.x} / Z ${selected.z}` : ""}`;
}
...
function ensureVillageViewControls(canvas) {
  ...
  canvas.addEventListener("pointerdown", (event) => {
    const decorationEditor = state.villageDecorationEditor;
    const position = villageCanvasWorldPosition(canvas, event);
    if (decorationEditor.tool === "place") {
      if (manualTownDecorations() === null) {
        setManualTownDecorations((decorationEditor.preview?.decorations || []).map((item) => ({
          type: item.type, x: Math.round(item.x), z: Math.round(item.z), rotation: item.rotation || "none"
        })));
      }
      const placements = manualTownDecorations();
      placements.push({
        type: $("#town-decoration-type").value,
        x: position.x, z: position.z,
        rotation: $("#town-decoration-rotation").value || "none"
      });
      decorationEditor.selected = placements.length - 1;
      decorationEditor.tool = "select";
      commitTownDecorationEdit();
      return;
    }
    const hit = [...decorationEditor.hitTargets].reverse().find((item) =>
      (item.x - position.screenX) ** 2 + (item.y - position.screenY) ** 2 <= 12 ** 2
    );
    if (hit && manualTownDecorations() !== null) {
      decorationEditor.selected = hit.index;
      decorationEditor.drag = { pointerId: event.pointerId };
  ...
  canvas.addEventListener("pointermove", (event) => {
    ...
    const screenX = (event.clientX - bounds.left) * canvas.width / bounds.width;
    const screenY = (event.clientY - bounds.top) * canvas.height / bounds.height;
    if (state.villageDecorationEditor.drag) {
      const position = villageCanvasWorldPosition(canvas, event);
      const selected = manualTownDecorations()?.[state.villageDecorationEditor.selected];
      if (selected) {
...
function renderVillageGenerationTest() {
  ...
  state.villageDecorationEditor.hitTargets = [];
  const viewport = ({
  ...
  for (const [decorationIndex, decoration] of (result.decorations || []).entries()) {
    ...
    if (configuredDecorations !== null && decorationIndex === state.villageDecorationEditor.selected) {
      context.strokeStyle = "#e23b32";
      context.lineWidth = 2;
      context.strokeRect(position.x - 7, position.y - 7, 14, 14);
    }
  }
  const labels = [];
...
function updateFacilityFormState(preferredFootprintShape = null) {
  const form = $("#settlement-form");
  ...
  // The select options may still belong to the previously opened settlement.
  // Use the newly loaded document value when rendering so a 5-cell/3-cell
  // transition cannot silently fall back to line_q.
  const previousShape = normalizeTownFootprintShape(preferredFootprintShape ?? shapeSelect.value);
...
function renderSettlement() {
  ...
  renderSharedTrainerPopulation("settlement");
  [...form.elements].forEach((element) => element.disabled = false);
...
function updateSettlementFromForm() {
  ...
  state.settlement.npc_placement.auto_place_npcs = form.elements.autoPlaceNpcs.checked;
  updateSharedTrainerPopulation("settlement");
  state.settlement.npc_placement.trainer_slots ||= [];
...
$("#encounter-direct-pokemon-list").addEventListener("change", (event) => {
  ...
  if (evolved) {
    const addition = settings.additions[Number(evolved.dataset.encounterSpawnAsEvolved)];
    if (addition) addition.spawn_as_evolved = evolved.checked;
    return;
  }
  const weight = event.target.closest("[data-encounter-spawn-weight]");
  if (weight) {
    const addition = settings.additions[Number(weight.dataset.encounterSpawnWeight)];
...
$("#route-encounter-method-tabs").addEventListener("click", (event) => {
  const button = event.target.closest("[data-route-encounter-method]");
  if (!button) return;
  state.routePokemonMethod = button.dataset.routeEncounterMethod;
  state.routePokemonLevelSpecies = null;
  state.routePokemonQuery = "";
  state.routePokemonPicker.selected.clear();
  $("#route-biome-pokemon-search").value = "";
  renderRoutePokemonDialog();
});
$("#route-encounter-method-enabled").addEventListener("change", (event) => {
  if (state.routePokemonMethod === "land") return;
  const settings = routePokemonSettings(); if (!settings) return;
  settings.enabled = event.target.checked;
  refreshRoutePokemonEditing();
});
$("#route-encounter-trigger-chance").addEventListener("change", (event) => {
  if (state.routePokemonMethod === "land") return;
  const settings = routePokemonSettings(); if (!settings) return;
  const percent = Math.max(0, Math.min(100, Number(event.target.value || 0)));
  settings.trigger_chance = percent / 100;
  refreshRoutePokemonEditing();
});
...
$("#route-biome-pokemon-list").addEventListener("click", (event) => {
  if (handlePokemonCardEditAction(event, "route", renderRoutePokemonDialog) || handlePokemonCardLevelAction(event, "route", renderRoutePokemonDialog)) return;
...
$("#route-direct-pokemon-list").addEventListener("change", (event) => {
  ...
  if (evolved) {
    const addition = settings.additions[Number(evolved.dataset.routeSpawnAsEvolved)];
    if (addition) addition.spawn_as_evolved = evolved.checked;
    return;
  }
  const weight = event.target.closest("[data-route-spawn-weight]");
  if (weight) {
    const addition = settings.additions[Number(weight.dataset.routeSpawnWeight)];
    if (addition) addition.weight = Math.max(1, Math.min(10000, Math.round(Number(weight.value || 1))));
    renderRoutePokemonDialog();
  }
});
...
$("#world-pokemon-generation-toggles").addEventListener("click", (event) => {
  event.preventDefault(); event.stopPropagation();
  const button = event.target.closest("[data-world-pokemon-generation]"); if (!button || !state.worldLayout) return;
  const generation = Number(button.dataset.worldPokemonGeneration);
  const selected = new Set(state.worldLayout.pokemon_generations || [state.selectedGeneration]);
  if (selected.has(generation)) {
    if (selected.size === 1) { toast("자동 출현 세대는 하나 이상 선택해야 합니다."); return; }
    selected.delete(generation);
  } else selected.add(generation);
  state.worldLayout.pokemon_generations = [...selected].sort((a, b) => a - b);
...
$("#town-decoration-convert").addEventListener("click", () => {
  ...
  setManualTownDecorations((state.villageDecorationEditor.preview?.decorations || []).map((item) => ({
    ...
    x: Math.round(item.x),
    z: Math.round(item.z),
    rotation: item.rotation || "none"
  })));
  state.villageDecorationEditor.selected = -1;
  commitTownDecorationEdit();
});
$("#town-decoration-add").addEventListener("click", () => {
  state.villageDecorationEditor.tool = "place";
  renderTownDecorationControls();
});
$("#town-decoration-delete").addEventListener("click", () => {
  const placements = manualTownDecorations();
  const index = state.villageDecorationEditor.selected;
  if (!placements || index < 0 || index >= placements.length) return;
  placements.splice(index, 1);
  state.villageDecorationEditor.selected = -1;
  commitTownDecorationEdit();
});
$("#town-decoration-rotation").addEventListener("change", (event) => {
  const selected = manualTownDecorations()?.[state.villageDecorationEditor.selected];
  if (!selected) return;
  selected.rotation = event.target.value;
  commitTownDecorationEdit();
});
