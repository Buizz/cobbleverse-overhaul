...
Iris의 기본 `R` 셰이더 새로고침 및 `O` 셰이더 선택 단축키는 플레이어 메뉴
클라이언트 초기화 시 `key.keyboard.unknown`으로 마이그레이션한다. 각각 기본값인
경우에만 해제하므로 사용자가 이미 지정한 키는 보존한다. 모드팩은 전역
`options.txt`를 덮어쓰지 않으므로 언어와 사용자의 다른 키 설정도 보존한다.
포켓파인더 HUD는 `O`를 사용하며, 같은 키를 기본값으로 사용하는 Cobblemon의
파티 정보 숨기기는 `[`로 옮긴다. 이 역시 `O`가 그대로 남아 있는 기존 설치본만
마이그레이션하므로 사용자가 직접 바꾼 키는 건드리지 않는다.
