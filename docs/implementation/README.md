...
1. 3안의 보호형 서바이벌 지역, 수확 작물, 재생 광물 던전, 집 PC, 1청크 개인 농장과 가방 인벤토리를 최소 규모로 연결한다.
2. 지역 경계, 관장 진행, 비전머신과 차원 왕복의 우회·복구 시나리오를 검증한다.
3. 트레이너, 경제와 보상 데이터를 첫 지역 수직 구간에 추가한다.
4. 개인 농장의 상시 로드, 성장 처리와 가방 아이템 이동을 부하 테스트한다.
