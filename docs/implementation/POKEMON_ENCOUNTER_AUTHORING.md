...
`worlds/generation_1.json`의 각 `tiles[]`는 선택적으로 `pokemon_habitat`을 가진다.

| `source` | 동작 |
|---|---|
| `biome` 또는 필드 생략 | 타일의 `biome` 기본 후보 사용 |
| `route` | `route_id`로 지정한 월드 길의 모든 조우 풀을 실시간 상속 |
| `custom` | 타일 안의 `pokemon_spawns`로 육상·파도타기·낚시·박치기 풀을 독립 설정 |

길 상속은 출현 설정만 공유하며 길 지형이나 좌표 소유권을 추가하지 않는다. 타일 전용
설정은 그 타일에서 실제 길 오버레이보다 우선한다.

### 3.2 낮·밤 오버라이드

길, 타일 전용 풀, 숲·동굴의 각 출현 설정은 `time_overrides`를 사용할 수 있다.
항목은 `species`와 `time`(`any`, `day`, `night`)을 가지며, 생략하면 해당 포켓몬의
원본 시간 조건을 유지한다. 조우 방식별 풀은 서로 독립이므로 같은 포켓몬이라도 육상과
낚시의 시간대를 다르게 정할 수 있다.
...
던전의 `random_encounters`에는 고정 출현 좌표나 범위를 작성하지 않는다. 런타임이
매 실행에서 생성된 던전 크기를 출현 한계로 자동 사용하고, 그 안에서 바닥과 머리
공간이 유효한 플레이어 주변 위치를 찾는다. 따라서 던전 구조나 시드가 바뀌어도
출현 범위를 별도로 다시 맞출 필요가 없다.
...
10. 낮·밤 오버라이드가 있으면 두 시간대에서 후보가 정확히 바뀌는지 확인한다.
11. 인게임에서 육상·물속·각 낚싯대·박치기를 각각 따로 확인한다.
...
| 던전에서 바이옴 포켓몬이 나옴 | 던전 `random_encounters`와 실행 시 자동 계산된 던전 범위 확인 |
