...
> 결정: BCA는 마을 길과 주택을 생성하고, 체육관과 리그는 Radical Gyms &
> Structures(RGS) 0.6의 원본 구조물을 지상에 직접 배치한다.

## 역할 분리

- `tools/mod-builder/starter_gym.py`는 더 이상 체육관 외관을 만들지 않는다. BCA의
  길·주택 Jigsaw를 시작하는 작은 중립 도로 허브만 생성한다.
- 실제 체육관 건물은 마을의 `gym_building` 방향을 기준으로 `rgs:*_gym` NBT를
  원형 그대로 직접 배치한다. 플레이어는 순간이동 없이 문으로 건물에 들어간다.
...
RGS 0.6 체육관의 공통 주 출입구는 템플릿 서쪽 면의 로컬 좌표
`(x=2, y=3, z=10)`이다. `gym_entrance_offset`은 이 값을 기록하며, 순환도로의
진입로는 구조물 중심이 아니라 이 출입구 바로 앞까지 이어져야 한다.

## 마을 JSON 계약

지상에 직접 배치할 체육관은 `structure_profile.facility_placements`에 선언한다.
...
- `direct_template`: RGS 체육관은 실제 템플릿 크기를 읽은 뒤 BCA 직소 최대 반경
...
2. BCA 마을 배치가 끝난 뒤 `direct_template` 체육관을 직소 최대 확장 반경 밖의
   예약 영역에 `/place template`으로 배치한다.
...
4. 하나라도 배치에 실패하면 지도 완료 상태를 저장하지 않아 다음 접속에서
   재시도할 수 있게 한다.
5. RGS 구조물의 명령 블록 동작을 사용하는 서버는 `enable-command-block=true`로
   실행한다.
...
- 다섯 마을에 지정된 RGS 체육관 건물이 지표면에 직접 생성된다.
