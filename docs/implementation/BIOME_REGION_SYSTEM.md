...
- 결정: `DEC-WORLD-014` — 세대 차원은 육각 계획 그래프와 연속 지역 마스크 및 전용 `BiomeSource`를 사용한다.
- 상태: 연속 마스크 부트스트랩 구현 / BiomeSource 이관 예정
...
| 세대 월드 JSON | 육각 계획 격자, 마을 앵커, 바이옴 영향 반경·굴곡·경계, 경로 그래프 |
| 마을 JSON | 셀 중심 기준 건물·NPC·시설 상대 배치와 건축 키트 |
...
사각형·원형·팔각형 범위 확장안은 사용하지 않는다. 모든 지역의 배치 계획은 동일한
육각 좌표계를 쓰지만, 실제 지형은 셀 중심 영향 마스크의 합집합과 저주파 노이즈로
표현한다. 통로 역시 A* 셀 경로를 중심선으로 삼는 연속 폭 마스크다.
