...
- 실제 크기, 허용 회전, 역할과 사용 횟수
- 외부 통로가 연결되는 `ports`
- 생성 벽·묘비·책상 등을 놓을 수 있는 `partition_zones`
- 항상 비워 둘 `walk_zones`와 전투 안전 영역
- 조우·보상·목표·장식 의미 마커
...
| `chamber_maze` | 하나의 공동 내부 구획으로 도달 가능한 미로를 생성 | 포켓몬타워 묘비층, 창고 선반 미로 |
...
통로와 루프를 추가한다. `chamber_maze`는 `partition_zones` 안에서만 벽과 장식을
생성하고 입구·목표·출구 사이의 도달 가능성을 마지막에 다시 검사한다.
