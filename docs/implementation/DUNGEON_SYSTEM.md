...
1. 조우의 NPC 수만큼 일반 통로 슬롯을 확보하도록 경로 길이를 늘린다. 일반 통로
   조각 하나는 방향과 무관하게 중앙의 NPC 슬롯 하나만 제공한다.
...
3. `spatial_layout.chamber_pieces`는 공동의 배치 목록이 아니라 사용할 수 있는 후보
   풀이다. 선택한 후보가 있으면 여러 층의 각 층에 최소 하나씩 배분하지만 NPC 수로
   공동을 증식시키지는 않는다. 공동의 `npc_spawn` 마커만 NPC 슬롯으로 사용하고,
   마커가 없는 공동은 NPC 배치에서 건너뛴다.
4. 각 층을 독립된 완성 평면으로 만든다. 방 사이에는 회전 가능한 공통 `corridor`
   계열 조각을 2개, 다음 구간에는 3개를 두는 간격을 반복한다.
5. 모든 층이 완성된 뒤 계단을 붙인다. 계단 방향이 맞지 않으면 다음 층 전체를
   90도 단위로 회전하고, 위층 생성 시 계단 점유 공간을 미리 예약한다.
6. 실제 회전된 3차원 경계를 한 블록 단위로 검사한다. 방·통로·계단이 겹치거나
...
생성기는 여러 층 던전의 각 층에 후보 공동을 최소 하나 배치한다. NPC는 공동의
`npc_spawn` 마커를 먼저 사용하고, 남은 NPC는 서로 다른 일반 통로 조각에 한 명씩
분산한다. 공동에 `npc_spawn` 마커가 없으면 그 공동은 NPC 배치에서 건너뛴다.
...
2. 전체 공동 수를 정하고 각 공동을 16×16 기준 점유 칸 수로 환산한다. 큰 필수
   ...
   자동 생성 층도 실제 공동을 최소 두 개 포함해야 한다. NPC 수로 추가 공동이
   필요하면 공동 하나와 그 공동으로 이어지는 통로 두 칸을 기본 구조에 더한다.
