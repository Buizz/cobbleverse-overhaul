...
절차 동굴 생성기는 생성한 주 경로 방과 곁가지 방의 안전한 바닥 중심을 콘텐츠 후보로 함께 반환한다. 좌표를 생략한 일반 조우는 주 경로 방, 보스는 출구에 가까운 마지막 주 경로 방, 상자와 조사 목표는 곁가지 방을 우선 사용한다. 한 방에는 중심과 네 방향 오프셋 슬롯을 제공하며 같은 실행 안에서 NPC·보상·목표가 같은 블록을 공유하지 않는다. 배치는 실행 시드에 대해 결정적이므로 복구와 미리보기에서 동일한 결과를 재현할 수 있다.
...
- `min_per_plan`, `max_per_plan`: 해당 변형 조각의 계획당 최소·최대 사용 횟수
- `placement_scope`: `any`, `critical_path`, `branch` 중 조각이 허용되는 경로 영역
- `forbid_adjacent_tags`: 출입구로 직접 연결할 수 없는 상대 조각 태그 목록
...
통로를 막는 `gate` 마커는 `connector`로 차단할 연결구를 지정한다. 계획 검증기는 해당 연결을 임시로 제거한 뒤 시작 방에서 도달 가능한 영역을 다시 계산한다. 관문 양쪽이 다른 영역으로 분리되지 않거나, 관문이 요구하는 조우·스위치·조사 목표가 잠긴 쪽에 놓이면 계획을 거부한다. 따라서 주 경로를 우회하는 루프 때문에 관문이 무의미해지는 경우와, 열기 위해 필요한 조건이 문 뒤에 생성되는 순환 잠금을 게시 전에 차단한다. 참조 없는 관문 후보는 실제 계획에서 사용 중이며 진행 영역을 분리하는 커넥터만 자동 배정 대상으로 삼는다.
...
```json
{
  ...
  "requirements": [
    { "type": "objective", "id": "security_switch" },
    { "type": "item", "item": "minecraft:tripwire_hook", "count": 1, "consume": true }
  ]
}
```

기존 `requires`는 조우 ID 목록이며 계속 지원한다. 새 `requirements`는 `encounter`, `objective`, `item` 조건을 함께 선언한다. 모든 조건을 충족해야 열리고 `consume: true`인 아이템은 참가자 전체 인벤토리에서 합산하여 관문이 열리는 순간 차감한다. 스위치와 조사 지점은 `objectives`에 선언하고, NBT의 `objective` 마커 또는 고정 좌표에 배치한다. 완료 목표와 열린 관문은 실행 상태에 저장되어 서버 재시작 후에도 복구된다.
...
같은 역할의 변형 조각은 가중치와 계획당 사용 횟수로 출현을 통제한다. 일반 장식 변형은 `min_per_plan: 0`, `max_per_plan: 256` 기본값을 사용하고, 희귀 보물방은 최대 1회, 필수 기믹 방은 최소·최대 모두 1로 지정한다. 런타임 생성기는 최대 횟수에 도달한 조각을 후보에서 제외하고, 완성 계획이 모든 최소 횟수를 충족하지 않으면 다른 생성 시도를 수행한다. 웹에서 직접 만든 계획도 같은 검증을 통과해야 게시할 수 있다.

기믹 조각은 `placement_scope`로 주 경로 전용 또는 곁가지 전용으로 제한한다. 또한 `forbid_adjacent_tags`는 양방향 규칙으로 판정하므로 한쪽 조각에만 금지 태그를 적어도 해당 태그를 가진 방과 직접 연결되지 않는다. 런타임 생성, 루프 추가, 게시형 계획 검증과 웹 미리보기가 모두 같은 규칙을 사용한다. 예를 들어 테스트용 로켓단 보물방은 곁가지에서만 출현하고 보스방 바로 옆에는 배치되지 않는다.
...
현재 런타임은 `nbt_pieces`에서 `authored`, `authored_pool`, `runtime`을 모두 실제 입장에 사용한다. `authored_pool`은 실행 시드로 게시 계획 하나를 결정적으로 선택한다. `procedural_cave`는 저장된 동굴 계획 포맷이 아직 없으므로 `runtime`만 허용하며 웹 편집기와 게시 검증에서도 다른 계획 방식을 선택하지 못하게 한다.
...
- 고정 던전 조우는 `requires`에 선행 조우 ID를 선언할 수 있다. 게시 검증은 존재하지
  ...
  될 때까지 후속 조우를 잠근다.
- `gates`는 닫힌 상태의 블록 범위와 조우·목표·아이템 조건을 선언한다. 실행 준비 시 해당
  범위를 잠금 블록으로 채우고 모든 조건이 충족되는 순간 그 실행 슬롯의 게이트만 제거한다.
