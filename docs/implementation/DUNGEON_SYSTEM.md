...
- 고정 NPC·보스는 `encounters`에 직접 작성하고, 일반 NPC 자동 생성은 던전 전역의 외형·대사·포켓몬 풀에서 실행마다 별도로 추첨한다.
...
| NPC 공간 | `npc_placement.capacity_mode` | 직접 수량 지정 또는 조우 액터 수에서 자동 계산 | `from_encounters` |
| NPC 공간 | `npc_placement.required_slots` | `fixed`일 때 지형이 반드시 제공할 안전 슬롯 수 | 직접 지정 시 필수 |
...
던전의 형태를 하나의 `layout.mode` 또는 `terrain.mode`로 모두 표현하지 않는다.
생성 설정은 다음 네 계층으로 분리한다.

```text
동선 계획(topology)
  → 수직 배치(vertical layout)
  → 공간 제작(construction)
  → 테마 표현(theme)
  → 콘텐츠 후보와 의미 마커가 포함된 DungeonPlan
```

| 계층 | 책임 | 다른 계층이 관여하지 않는 항목 |
|------|------|-------------------------------|
| `topology` | 시작·목표·출구, 주 경로, 방·공동·곁가지의 연결 그래프 | 블록 종류, NBT 파일, 실제 Y 좌표 |
| `vertical` | 평면, 연속 경사, 층별 배치와 층간 연결 | 방 역할, 장식, 조우 내용 |
| `construction` | 그래프와 공간 예약을 실제 블록·NBT 배치로 변환 | 진행 조건, 보상, NPC 팀 |
| `theme` | 바닥·벽·천장·조명 팔레트 또는 호환 NBT 스킨 선택 | 동선과 층 구조 |

모든 제작 방식은 블록을 설치하기 전에 공통 `DungeonPlan`을 반환한다. 계획은
최소한 다음 정보를 포함한다.

- 노드: `id`, `role`, `zone`, `floor`, 실제 경계, 회전, 주 경로 여부
- 연결: 양쪽 노드와 포트, 수평·계단·엘리베이터·경사 연결 종류
- 점유 공간: 구조물 경계뿐 아니라 문 앞, 계단, 통로와 전투 안전 공간
- 이동 영역: 걸을 수 있는 셀과 반드시 비워야 하는 주 동선
- 콘텐츠 후보: 조우, 보스, 전리품, 목표, 치료소, 체크포인트용 슬롯
- 배치 명령: NBT 배치, 절차 블록 채우기, 장식 규칙 적용 순서
- 재현 정보: 데이터 버전, 생성기 버전, 시드와 선택된 테마

계획 검증은 제작 방식과 무관하게 시작에서 보스와 출구까지의 도달 가능성,
층간 연결, 공간 충돌, 열린 포트, 필수 콘텐츠 수용량과 안전 공간을 검사한다.
검증을 통과한 동일 계획을 웹 미리보기와 실제 슬롯 생성이 함께 사용한다.

현재 구현의 `layout.mode`와 `terrain.mode`는 이 분리가 적용되기 전의 호환
필드다. 마이그레이션 중에는 기존 던전을 아래와 같이 해석한다.

```text
nbt_pieces + critical_path_branches
  = corridor_spine + continuous_vertical + modular_nbt

procedural_cave
  = natural_network + continuous_vertical + natural_cave

fixed_template
  = authored + authored_vertical + fixed_template
```

### 4.1 지형 생성 방식

지형 방식은 목표 모델에서 `construction.mode`가 담당한다. 기존
`terrain.mode`는 기존 콘텐츠를 깨뜨리지 않기 위해 유지하되 새 편집기에서는
공간 제작 방식으로 표시한다.

| `construction.mode` | 설명 | 주 용도 |
|---------------------|------|---------|
| `fixed_template` | 완성된 NBT 지역 하나를 그대로 사용 | 작은 발전소, 전설 연출, 정교한 보스방 |
| `modular_nbt` | 연결 포트가 있는 방·통로 NBT를 그래프에 맞춰 조립 | 하수구, 기지 통로, 유적 |
| `parametric_building` | 방 그래프를 바닥·벽·천장·조명 블록으로 직접 시공 | 사무실, 연구소, 범용 시설 |
| `chamber_composite` | 실제 크기를 예약한 공동 NBT를 절차 통로·층·구획과 합성 | 로비 중심 시설, 포켓몬타워, 대형 사무실 |
| `natural_cave` | 일반 동굴과 같은 공동·통로 굴착 규칙 사용 | 자연 동굴, 광산, 야생 서식지 |
| `cave_landmarks` | 자연동굴의 안전한 공동에 NBT 랜드마크 삽입 | 자연 동굴 속 사당, 유적과 보스방 |
...
`parametric_building`은 완성 NBT 조각을 요구하지 않는다. 필수 테마 입력은
`floor`, `wall`, `ceiling`, `light` 네 종류이며 문 개구부와 계단은 생성기가
만든다. 선택 입력으로 문, 창, 테두리와 장식 규칙을 덮어쓸 수 있지만 필수로
요구하지 않는다.

생성 순서는 외곽과 층별 바닥 예약, 방 구획, 통로와 문 개방, 층간 연결,
천장과 조명, 장식, 의미 마커 산출 순이다. 각 단계는 같은 점유 지도와 이동
지도를 사용해야 하며 장식 때문에 주 동선이나 전투 안전 공간이 막히면 계획을
거부한다.

#### 공동 NBT 합성

NBT는 확대·축소하지 않는다. `chamber_composite`는 구조물 파일의 실제 크기를
읽고 그 크기와 회전된 경계를 계획에 먼저 예약한 뒤 주변 방과 통로를 배치한다.
따라서 “크기에 맞게 배치”한다는 것은 NBT를 늘이는 것이 아니라 생성된 건물이나
층이 해당 NBT의 크기를 수용하도록 만드는 것을 뜻한다.

공동 정의는 다음 계약을 가진다.

- NBT에서 읽은 실제 크기와 16x16 기준 격자 점유 크기
- 허용 회전, 역할과 사용 횟수
- 외부 통로가 연결되는 `ports`
- 조우·보상·목표·장식 의미 마커
- 다른 공동과 같은 층에 배치할 수 있는지와 허용 층 역할

공동의 경계와 내부 사용 영역을 에딧월드에서 다시 그리지 않는다. NBT 경계
자체가 공동의 경계다. 벽·바닥·천장·조명·책상·묘비·기계·고정 상자·내부
칸막이와 기믹은 완성된 공동 NBT 안에 제작한다. 생성기는 공동 내부에 이러한
오브젝트를 임의로 흩뿌리지 않고, 의미 마커에 NPC·보스·전리품·목표만
할당한다.

공동 X/Z 크기는 16의 배수여야 하며 각각의 회전된 실제 크기에서 격자 점유
크기를 자동 계산한다. Y는 격자 크기로 고정하지 않고 NBT의 실제 높이를 쓴다.
동일한 격자 점유와 포트 계약을 가진 여러 NBT를 변형으로 등록하여 내부 형태의
다양성을 만든다.

연결 포트는 각 16×16 하위 격자의 통로 중앙선에 맞춘다. 제작자는 `north`,
`south`, `east`, `west` 포트를 직접 작성하지 않는다. 관리 도구와 빌드 과정이
NBT 종류·크기·기본 회전에서 내부 연결 정보를 계산한다. 직선 연결은 하나의
회전 가능한 `corridor` NBT를 공통으로 사용한다.

통로 배치기는 `corridor`, `corner`, `t_junction`, `junction` 역할이나 기본 방향을
먼저 확정하지 않는다. 경로가 해당 16×16 칸에 요구하는 열린 면을 먼저 구한 뒤,
같은 조각 풀의 모든 `passage` NBT를 0/90/180/270도로 회전해 정확히 맞는 조각을
선택한다. 역할과 모양 태그는 여러 조각이 모두 맞을 때의 선호도일 뿐 후보를
제외하는 조건이 아니다.

공동 여러 개를 배치할 때는 각 NBT의 구조물 경계뿐 아니라 포트 앞 접근 공간과
이동 영역도 충돌 검사에 포함한다. 기존 `hybrid`처럼 공동 중심에 NBT 원점을
맞추는 방식은 자연동굴 랜드마크 호환 모드로만 유지한다.

### 4.2 공통 방 네트워크

NBT 조각 던전은 진행 규칙과 층 평면을 별도로 선택한다. `progression.pattern`은
선형·분기·열쇠처럼 플레이 순서만 정하고, `spatial_layout.algorithm`은 모든 층에
공통으로 적용할 방 배치 형태만 정한다. 사용자에게 제공하는 평면은 다음 두 가지다.

NPC 전투 순서는 별도인 `progression.encounter_order`로 정한다. 기본값
`boss_only`에서는 일반 NPC를 어떤 순서로 만나거나 건너뛸 수 있고 보스 격파만
클리어에 필요하다. 일반 NPC를 요구하던 관문도 이 모드에서는 막지 않지만, 보스,
장치, 열쇠와 아이템을 요구하는 관문은 그대로 동작한다. `sequential`을 선택하면
각 조우의 `requires`와 일반 NPC 관문 요구를 기존처럼 모두 적용한다.

- `room_scatter`: 공동을 2차원에 분산 배치하고 공동 사이를 2~3칸 통로로 연결한다.
- `corridor_halls`: 공유 중심 복도를 만들고 위·아래에 공동을 붙이는 건물형 평면이다.

`corridor_halls`의 중심 복도는 한 층 안에서 앞뒤로 왕복하지 않는 직선 축이다.
NPC 공동과 선택 곁가지는 이 축에서 수직으로 짧게 뻗으며, 연속된 층은 가로축과
세로축을 교대로 사용한다. 시드가 바뀌면 전체 축의 반전과 공동 간격·가지 방향도
함께 달라진다. 계단 착지실은 4방향 연결 공동을 우선 사용해 계단 조각만 회전하며,
계단 방향 때문에 이미 완성된 위층의 축을 불필요하게 다시 돌리지 않는다.

과거 `socket_accretion` 값은 `room_scatter`로 읽는 호환값이며 새 저장에는 사용하지
않는다. 층마다 서로 다른 평면 형식을 지정하는 옵션은 두지 않는다.

`topology.loop_chance`는 이미 정해진 진행 그래프에 선택적인 지름길을 추가하는
확률이며, 진행 형식을 순환형으로 바꾸는 값이 아니다. `progression.pattern`이 없는
콘텐츠는 주 경로와 곁가지 수만으로 `linear` 또는 `branching`을 선택한다. 또한
시작·보스·출구처럼 사용할 수 있는 포트 수가 제한된 공동에 연결이 몰리면, 초과
연결은 공동에 직접 붙이지 않고 회전 가능한 통로 교차로를 한 칸 두어 분산한다.
...
3. `spatial_layout.chamber_pieces`는 공동의 배치 목록이 아니라 사용할 수 있는 후보
   풀이다. 후보별 `npc_spawn` 마커와 최소 간격을 적용한 안전 수용량을 측정하고,
   실제 일반 NPC를 수용하는 데 필요한 만큼만 공동 수를 정한다. NPC가 배치되는
   층에는 최소 한 공동을 배정하되 빈 자동 공동은 만들지 않는다. 마커가 없는 공동은
   NPC 배치용 후보에서 제외한다.
4. 각 층을 독립된 완성 평면으로 만든다. 방 사이에는 회전 가능한 공통 `corridor`
   계열 조각을 2개, 다음 구간에는 3개를 두는 간격을 반복한다.
5. 공동과 기본 통로가 제공하는 실제 NPC 용량을 제외하고도 부족한 수량만 일반
   통로 후반부에 한 자리씩 추가한다. `fixed` 예비 용량도 이 후반부에만 붙이며
   공동 수나 실제 NPC 배치 수를 늘리지 않는다.
6. 모든 층이 완성된 뒤 계단을 붙인다. 계단 방향이 맞지 않으면 다음 층 전체를
   90도 단위로 회전하고, 위층 생성 시 계단 점유 공간을 미리 예약한다.
7. 실제 회전된 3차원 경계를 한 블록 단위로 검사한다. 방·통로·계단이 겹치거나
   열린 연결구를 안전하게 마감할 수 없으면 해당 시도는 폐기한다.

중·대형 공동은 기준점 한 칸이 아니라 회전된 전체 점유 칸을 경로 탐색 전에
예약한다. 통로의 시작과 끝은 공동 경계의 임의 칸이 아니라 실제 포트가 포함된
외곽 칸과 그 바로 바깥 칸으로 고정한다. 경로가 공동 내부를 지나거나, 포트가 없는
벽에 닿거나, 통로 조각 사이의 연결 좌표가 끊긴 계획은 생성 결과로 사용할 수 없다.

자동 경로의 일반 진행 노드는 통로 조각으로 배치한다. `chamber_pieces`에서 명시적으로
선택한 공동만 크기와 포트에 맞는 진행 노드를 대체하며, 선택 목록이 비어 있으면
일반 공동을 자동으로 고르지 않는다. 시작·보스·출구 같은 필수 단말 공간은 이 목록과
무관하게 유지한다. 배치된 공동의 실제 개구부는 모두 추적한다. 경로에 사용한 개구부는 통로에 연결하고, 남은 개구부는
회전 가능한 한 칸짜리 종단 조각으로 닫는다. 입구와 층간 계단에 예약한 개구부만
이 마감 대상에서 제외한다. 서로 다른 경로가 같은 격자 칸에서 만나면 통로를
겹쳐 놓지 않고 그 칸의 열린 면을 합쳐 `t_junction` 또는 `junction` 하나를 배치한다.
필요한 공동을 배치할 공간이 없으면 겹친 결과를 허용하지 않고 생성에 실패한다.

#### 진행 그래프

먼저 블록과 좌표가 없는 진행 그래프를 만들고 공통 방 네트워크가 그래프 노드를
실제 NBT 방과 통로로 바꾼다.

진행 그래프의 기본 패턴은 다음과 같다.

| 패턴 | 그래프 보장 |
|------|-------------|
| `linear` | 시작, 주 경로, 보스, 출구가 한 경로로 연결됨 |
| `branching` | 주 경로와 선택 보상 가지가 분리됨 |
| `cyclic` | 우회 경로가 뒤의 주 경로에 다시 합류함 |
| `parallel_gate` | 여러 목표가 각각 접근 가능하고 모두 완료해야 보스에 접근함 |
| `key_lock` | 열쇠가 잠긴 노드보다 먼저 획득 가능함 |
| `shortcut_loop` | 진행 후 앞 구역으로 돌아가는 지름길이 열림 |

제작자는 `spatial_layout.chamber_pieces`에서 이번 던전에 사용할 수 있는 일반 공동
NBT 후보를 고른다. 이 목록은 종류를 정할 뿐 각 후보를 한 번씩 강제 배치하지 않는다.
생성기는 실제 일반 NPC 수와 공동별 안전 수용량으로 전체 공동 수를 정하고 NPC가
배치되는 층에 고르게 나눈다. NPC는 공동의 `npc_spawn` 마커를 먼저 사용하고, 남은
NPC는 서로 다른 일반 통로 조각에 한 명씩 분산한다. 공동에 `npc_spawn` 마커가
없으면 그 공동은 NPC 배치에서 건너뛴다.
목록이 비어 있으면 일반 공동 수는 0개다.
시작·보스·출구 공동은 역할상 자동으로 필요하므로 이 목록에서 다시 선택하지 않는다.
공동에 NPC 자리를 만들 때는 마커 ID를 `npc1`, `npc2`처럼 작성하고 `kind`는
`npc_spawn`으로 지정한다. ID의 숫자는 제작자용 식별자이며 실제 배치 우선순위는
마커 간 거리와 던전 전체 분산 상태로 결정한다.
...
공동과 통로는 실제 NBT 경계의 3차원 점유 영역을 공유한다. 이미 예약된 영역을
한 블록이라도 침범하는 후보는 즉시 제외하고, 하나의 연결구를 두 경로가 함께
사용하거나 기존 통로 위에 새 통로를 겹쳐 놓는 계획도 거부한다. 유효한 배치를
찾지 못하면 겹친 결과를 생성하지 않고 다른 시드로 재시도하거나 입장을 거부한다.

#### 수직 배치

수직 구조는 동선 형태와 독립된 `vertical.mode`로 지정한다.

| `vertical.mode` | 규칙 | 대표 용도 |
|-----------------|------|-----------|
| `flat` | 모든 일반 공간을 한 높이에 배치 | 단층 기지, 평면 미로 |
| `continuous` | 연결 포트의 실제 Y를 따라 경사·계단·낙차가 연속됨 | 하수구, 파이프, 갱도 |
| `discrete_floors` | 일반 공간은 정해진 층 Y에만 놓이고 전용 계단·엘리베이터만 층을 연결 | 실프주식회사, 포켓몬타워 |
| `authored` | 완성 계획 또는 NBT가 Y 배치를 소유 | 고정 템플릿 |

`discrete_floors`는 최소한 `floor_count`, `floor_height`, 층별 허용 경계와
층간 연결 수를 가진다. 층 Y는 조각이 우연히 높아진 값이 아니라
`base_y + floor * floor_height`로 계산한다. 일반 방은 해당 높이에 정렬하고,
계단과 엘리베이터만 정확히 인접한 두 층의 포트를 연결한다.

`discrete_floors`의 생성 순서는 다음으로 고정한다.

1. 필수 공동과 자동 공동의 실제 회전 크기 및 수평 점유 면적을 측정한다.
2. 전체 공동 수를 정하고 각 공동을 16×16 기준 점유 칸 수로 환산한다. 큰 필수
   공동부터 누적 점유 칸이 가장 작은 층에 배정하고, 큰 공동이 들어간 층에는 그
   면적만큼 일반 공동을 덜 배정해 층별 실제 사용 면적을 균등하게 맞춘다.
   자동 공동은 실제 NPC가 배치되는 층에만 최소 하나를 보장한다. NPC 수로 추가
   공동이 필요하면 공동 하나와 그 공동으로 이어지는 통로 두 칸을 기본 구조에
   더하며, NPC가 없는 빈 공동을 층수만 맞추기 위해 만들지 않는다.
3. 각 층을 로컬 좌표에서 독립적으로 완성한다. 이 단계에서는 다른 층이나 계단
   NBT를 아직 배치하지 않으며, 이전·다음 층에 사용할 연결 포트만 예약한다.
4. 모든 층이 유효할 때만 인접 층 사이에 계단을 배치한다. 계단을 이전 층의 예약
   포트에 회전해 붙이고, 반대편 포트 방향이 맞지 않으면 다음 층 전체를
   0/90/180/270도 회전한 뒤 이동해 정렬한다.
5. 회전·이동된 모든 NBT의 실제 3차원 경계, 포트 위치, 전체 던전 경계와 열린
   연결구를 다시 검증한다. 하나라도 실패하면 완성되지 않은 결과를 사용하지 않고
   전체 계획을 다시 생성한다.

층 내부 생성은 모든 층에 같은 알고리즘을 사용한다. 층마다 별도 형식이나 별도
알고리즘을 고르는 옵션은 두지 않는다.
...
던전은 `random_encounters`에서 직접 출현 풀을 받는다. `additions`에 종·레벨·가중치를 지정하며, 출현 범위는 고정 좌표를 작성하지 않고 매 실행에서 생성된 던전 전체 크기로 자동 계산한다. 그 안에서 플레이어 주변의 바닥과 머리 공간이 유효한 위치만 실제 후보가 된다. `max_active`와 `spawn_interval_ticks`는 실행 슬롯별 개체 수와 재생성 속도를 제어한다. 이후 프로필 참조를 추가해도 이 직접 목록은 던전별 덮어쓰기로 유지한다.
...
던전이 먼저 만들어진 뒤 남는 좌표에 NPC를 억지로 겹쳐 넣지 않는다. 지형 계획
단계에서 필요한 NPC 슬롯 수를 생성 목표로 사용하며, 완성된 계획의 실제 안전
슬롯 수가 목표보다 적으면 해당 계획은 컴파일에 실패한다.

```json
{
  "npc_placement": {
    "capacity_mode": "from_encounters",
    "minimum_spacing": 4
  }
}
```
...
| `capacity_mode` | 입력 | 동작 |
|-----------------|------|------|
| `fixed` | `required_slots` | 제작자가 입력한 수만큼 NPC 배치 가능 공간을 지형에 확보한다. 실제 NPC 수보다 작게 입력하면 콘텐츠 컴파일 오류다. |
| `from_encounters` | 없음 | 모든 조우의 실제 NPC 액터 수를 합산해 필요한 슬롯 수를 자동 결정한다. |

현재처럼 모든 적 NPC를 실행 시작 때 배치하는 던전은 일반 조우와 보스의 액터를
모두 합산한다. 향후 특정 웨이브 종료 후 NPC를 확실히 제거하고 같은 공간을 다시
쓰는 형식을 추가할 때만 `simultaneous_group` 단위의 최대 동시 수로 계산할 수
있다. 제거가 보장되지 않은 두 조우가 같은 슬롯을 공유해서는 안 된다.

하나의 NPC 슬롯은 단순한 마커 좌표가 아니라 다음 검사를 통과한 배치 가능
공간이다.

- NPC가 설 바닥과 머리 공간이 안전하고 낙하·물·용암 위가 아니다.
- 입구, 출구, 문, 연결 포트, 계단, 엘리베이터와 필수 주 동선을 막지 않는다.
- 다른 NPC와 `minimum_spacing` 이상 떨어지고 방별 최대 수를 넘지 않는다.
- 해당 조우를 시작할 플레이어의 접근 경로와 전투 시작 대기 공간이 남아 있다.
- 협력 조우는 NPC 두 자리뿐 아니라 플레이어 두 명의 집결 자리도 함께 확보한다.
- 조우 단위로 묶인 NPC는 서로 다른 방에 흩어지지 않고 허용 거리와 시야 방향을
  만족한다.

NBT 조각과 공동 NBT는 명시적인 NPC 마커 여러 개 또는 NPC 배치 가능 영역을
제공한다. `parametric_building`과 자연동굴은 이동 영역에서 위 조건을 만족하는
후보를 산출한다. 마커가 많더라도 서로 너무 가깝거나 같은 전투의 플레이어 공간을
침범하면 실제 수용량에는 포함하지 않는다.

검증은 다음 순서로 수행한다.

```text
던전 정의에서 NPC 수요 계산
  → 생성기가 최소 슬롯 수를 만족하도록 방·공동 배치
  → 실제 블록 경계와 이동 영역에서 안전 슬롯 산출
  → 조우 그룹을 슬롯에 배정
  → 남은 조우가 있으면 DungeonPlan 컴파일 실패
```

`authored`와 `authored_pool`은 저장·게시 시 즉시 오류로 막는다. `runtime`은
수용량이 부족한 결과를 유효한 계획으로 취급하지 않고 다음 생성 시도를 수행한다.
최대 시도까지 모두 실패하면 빈 공간에 NPC를 배치하거나 수를 줄이지 않고 던전
준비를 실패시킨다. 게시 검증에서는 조각·공동 풀과 전체 경계가 요구 수용량을
이론상 만들 수 있는지 검사하고, 검증 시드 묶음과 안전 대체 계획도 같은 수용량
검사를 통과해야 한다.

트레이너는 용도에 따라 두 영역으로 나눈다.
...
```json
{
  "generated_trainers": {
    ...
    "count": [4, 7],
    "appearance_pool": [
      { "display_name": { "ko_kr": "로켓단 조무래기" }, "trainer_class": "cobbleventure:trainer_class/villain_grunt", "weight": 4 },
      { "display_name": { "ko_kr": "로켓단 과학자" }, "trainer_class": "cobbleventure:trainer_class/scientist", "weight": 1 }
    ],
    "dialogue_pool": [
      { "battle_start_line": "로켓단의 계획을 방해하지 마라!", "battle_end_line": "이럴 수가...", "weight": 2 },
      { "battle_start_line": "여기서부터는 못 간다!", "battle_end_line": "간부님께 보고해야겠어!", "weight": 1 }
    ],
    "pokemon_pool": [
...
- 대사는 시작·종료 한 쌍을 하나의 가중치 항목으로 보관해 서로 어울리지 않는 문장이 임의로 섞이지 않게 한다.
- 던전 실행 시드로 NPC 수, 외형, 대사, 팀을 결정한다. 같은 실행을 복구하면 같은 결과를 얻고 새 실행에서는 달라질 수 있다.
- 중복을 금지한 상태에서 포켓몬 풀의 고유 종 수보다 최대 팀 크기가 크면 게시를 막는다.
- `count`의 최대값은 지형 생성 전 NPC 슬롯 수요에 반영한다. 실제 추첨 수보다 넉넉한 공간을 확보하므로 NPC 수를 늘리면 공동 수도 함께 늘어날 수 있다.
- 웹 편집기는 자동 NPC 생성을 내부 콘텐츠 밖의 독립 카드로 표시한다. 내부 콘텐츠에는 수동으로 만든 고정 NPC와 보스만 보인다.
...
미리보기는 원본 설정을 별도로 흉내 낸 결과가 아니라 생성된 `DungeonPlan`을
보여 준다. 현재 웹의 `runtime` NBT 미리보기는 서버 생성기를 JavaScript로
단순 모사하고 `exact: false` 결과를 만들기 때문에 이 계약을 아직 충족하지
못한다. 새 생성 형태를 추가하기 전에 계획 생성기를 독립 코어로 분리하고 웹
관리 서버가 그 코어에 같은 설정과 시드를 요청하도록 변경한다. 브라우저는 계획을
생성하지 않고 반환된 계획을 표시·수정하는 역할만 맡는다.
...
```json
{
  ...
  "construction": {
    "mode": "chamber_composite",
    "chamber_pool": "cobbleventure:rocket_office_chambers",
    "bounds": [160, 48, 160]
  },
  "topology": {
    "mode": "room_network",
  ...
  "vertical": {
    "mode": "discrete_floors",
    "floor_count": [3, 4],
    "floor_height": 8,
    "connections_per_floor": [1, 2]
  },
  "theme": {
    "mode": "block_palette",
...
- `npc_placement.capacity_mode: fixed`의 `required_slots`가 실제 NPC 액터 수보다
  작지 않으며, `from_encounters`는 고정 콘텐츠·자동 생성 NPC와 보스를 포함한
  실제 수요를 정확히 계산한다.
- 완성 계획의 고유한 안전 NPC 슬롯 수가 요구 수 이상이고, 모든 조우 그룹을
  최소 간격·방별 제한·플레이어 대기 공간 조건에 맞게 배정할 수 있다.
- 협력 조우 사이트마다 상대 NPC 수와 참가 플레이어 수에 맞는 별도 위치가 있고
  문·계단·주 동선 및 다른 조우 사이트와 겹치지 않는다.
...
### 4단계: 생성 코어 재구성과 형태 확장

- 기존 `DungeonPiecePlan`을 NBT 전용 배치 목록에서 제작 방식과 무관한 공통
  `DungeonPlan`으로 확장한다. 노드, 연결, 층, 점유 영역, 이동 영역, 콘텐츠
  후보와 배치 명령을 분리한다.
- 기존 동작을 `corridor_spine + continuous + modular_nbt` 호환 어댑터로
  먼저 옮기고 동일 시드 계획이 바뀌지 않는 회귀 테스트를 작성한다.
- `discrete_floors`를 구현해 층고, 층별 수평 경계, 계단·엘리베이터 포트와
  층별 도달 가능성을 명시적으로 검증한다.
- NBT 조각 던전은 공통 `room_network` 계획기로 통합한다. 기존 `maze`,
  `rooms_and_corridors`, `hub_and_spokes` 값은 호환 변환 후 사용자 옵션에서
  제거한다.
- `parametric_building` 렌더러로 바닥·벽·천장·조명 팔레트만 사용한 시설을
  시공하고, 같은 계획에 NBT 공동을 합성하는 `chamber_composite`를 추가한다.
- 자연동굴은 기존 일반 동굴 데이터와 생성기를 계속 사용한다. 자연동굴의 NBT
  랜드마크 기능은 `cave_landmarks`로 분리해 건물 공동 합성과 혼동하지 않는다.
- 여러 시드에서 도달 가능성, 층간 연결, 이동 폭, 콘텐츠 수용량, 생성 시간,
  청크 경계와 서버 부하를 자동 시험한다.
- NPC 액터 수를 계획 입력으로 전달하고, 생성기가 요구 슬롯을 확보한 뒤 실제
  조우 그룹 배정까지 성공해야만 계획을 승인하도록 한다.

### 5단계: 제작 경험 완성

- 웹의 독립적인 근사 생성기를 제거하고 관리 서버가 공통 계획 코어에서 받은
  정확한 `DungeonPlan`을 미리보기에 전달한다.
- 진행 구조, 층 수, 사용할 공동과 테마를 간단 옵션으로 제공한다. 공간 배치는
  모든 층에 공통인 `공동 분산형`과 `중앙 복도형`만 노출하고 세부 배치 수치는
  숨긴다.
- 층별 보기, 전체 층 겹쳐 보기, 공동 경계·포트·구획 가능 영역·이동 영역과
  예상 콘텐츠 배치를 같은 계획 위에 표시한다.
