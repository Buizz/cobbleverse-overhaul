...
기존 `layout.mode`처럼 모든 형태를 같은 연결 탐색기의 역할 선택으로 처리하지
않는다. 각 `topology.mode`는 별도의 계획 알고리즘과 검증 규칙을 가진다.

| `topology.mode` | 보장 사항 | 대표 용도 |
|-----------------|-----------|-----------|
| `authored` | 제작자가 지정한 방과 연결을 그대로 사용 | 연출형 던전, 수동 계획 |
| `corridor_spine` | 시작→보스→출구 주 통로와 선택적 곁가지 | 하수구, 파이프, 좁은 기지 |
| `hub_and_spokes` | 하나 이상의 대공동을 먼저 배치하고 주변 작은 방을 연결 | 중앙 로비, 제어실 중심 시설 |
| `room_network` | 방을 먼저 공간에 배치하고 필요한 방 사이를 통로로 연결 | 사무실, 연구소, 지하기지 |
| `natural_network` | 자연 공동과 굴곡 통로를 생성 | 동굴, 광산 |

`hub_and_spokes`는 대공동의 사용 가능한 포트 수보다 주변 연결 요구량이 많으면
계획을 거부하거나 다른 공동 변형을 선택한다. `room_network`는 의미 없는 방을
통로 사이에 넣지 않고 방 배치 후 최소 신장 연결을 만든 뒤 설정된 수만큼 우회
통로와 루프를 추가한다.

`chamber_maze`는 던전 전체의 `topology.mode`가 아니다. 기본 공동 내부는
`interior.mode: authored`로서 NBT를 그대로 사용한다. 작은 미로가 필요한 특정
공동만 `interior.mode: small_maze`와 NBT 안의 `maze_area` 마커를 선언한다.
이 예외 모드는 지정된 영역 안에서만 칸막이를 만들며 입구·출구·필수 이벤트
공간은 변경하지 않는다.

#### 진행 그래프와 공간 배치 알고리즘

동선 계획은 진행 규칙과 공간 배치를 한 알고리즘에서 동시에 만들지 않는다.
먼저 블록과 좌표가 없는 진행 그래프를 만든 뒤, 선택된 공간 배치기가 그래프의
노드를 공동과 방에 대응시킨다.
...
공간 배치기는 진행 그래프와 독립적으로 선택한다.

| 배치기 | 핵심 절차 | 대표 용도 |
|--------|-----------|-----------|
| `grid_walk` | 격자에 보장된 주 경로를 먼저 놓고 가지를 붙임 | 좁은 기지, 선형 던전 |
| `socket_accretion` | 배치된 공동의 열린 포트에 다음 공동을 누적 배치 | 유적, 유기적 기지 |
| `scatter_graph` | 공동을 분산 배치하고 최소 연결과 일부 루프를 만든 뒤 포트 사이를 라우팅 | 광산, 공동 군집 |
| `bsp_floor` | 한정된 층 외곽을 분할하고 크기에 맞는 공동을 채움 | 사무실, 연구소 |
| `authored` | 제작자가 지정한 공동 배치와 연결을 사용 | 연출형 던전 |

`Delaunay`와 최소 신장 트리는 독립적인 던전 형태가 아니라 `scatter_graph`가
사용하는 연결 후보 선택 단계다. 최소 신장 트리만 사용하면 모든 곁가지가
막다른 길이 되므로 의미 있는 후보 연결을 일부 복구해 루프를 만든다. 반대로
WFC와 완전 미로 알고리즘은 전역 진행을 만들지 않으며, 각각 국소 조각 선택과
`small_maze` 공동 내부에만 제한한다.

모든 공간 배치기는 같은 진행 그래프를 입력받고 같은 `DungeonPlan`을 출력한다.
따라서 `parallel_gate + bsp_floor`와 `parallel_gate + socket_accretion`처럼 진행
규칙을 유지한 채 공간 형태만 바꿀 수 있어야 한다.
...
계단과 엘리베이터만 정확히 인접한 두 층의 포트를 연결한다. 여러 층은 설정된
건물 수평 경계 안에 겹쳐 배치하며, 각 층마다 입구에서 다음 층 연결점까지의
유효 동선을 검사한다.
...
- `npc_placement.capacity_mode: fixed`의 `required_slots`가 실제 NPC 액터 수보다
  작지 않으며, `from_encounters`는 고정·즉석 생성 트레이너와 보스를 포함한
  실제 수요를 정확히 계산한다.
...
- `room_network`, `hub_and_spokes`, `chamber_maze`를 서로 독립된 계획기로
  구현한다. 단순히 역할 후보만 바꾸는 기존 `maze`·`rooms_and_corridors` 분기는
  호환 변환 후 제거한다.
...
- 생성 형태, 수직 방식, 공간 제작 방식과 테마를 독립된 간단 옵션으로 제공하고
  알고리즘별 세부 값은 상세 옵션으로 접는다.
