...
| NPC 공간 | `minimum_spacing`, `maximum_per_room` | NPC 밀집과 이동 방해를 방지하는 공간 제약 | 프리셋 상속 |
...
1. 조우의 NPC 수와 `npc_placement.maximum_per_room`, 공동의 `npc_spawn` 마커 수로
   필요한 NPC 공동 수를 먼저 계산한다. NPC 수가 늘면 주 경로 길이를 세 배로
   늘리지 않고, 계산된 수만큼 NPC 공동을 각 층의 짧은 곁가지에 분산한다.
2. 공동의 실제 X/Z 크기를 16×16 점유 칸으로 환산해 소공동 1칸, 중형 공동 2~3칸,
   대공동 4칸 이상으로 자동 분류한다. 제작자가 크기 등급을 따로 입력하지 않는다.
3. `spatial_layout.chamber_pieces`에서 선택한 공동을 큰 순서대로 층에 배분한다.
   큰 공동이 차지한 면적만큼 그 층의 일반 방 수를 줄여 층별 사용 면적을 맞춘다.
...
선택 공동은 계획에서 빠질 수 없으며, 배치할 공간이 없으면 겹친 결과를 허용하지
않고 생성에 실패한다.
...
제작자는 `spatial_layout.chamber_pieces`에서 이번 던전에 반드시 사용할 일반 공동
NBT를 고를 수 있다. 선택한 공동은 각각 계획에 한 번 이상 포함되어야 하며,
실제 회전 크기와 연결 포트를 먼저 예약한 뒤 통로를 붙인다. 선택 목록은 자동 후보에
추가하는 목록이 아니라 해당 던전에서 사용할 수 있는 일반 공동의 명시적 목록이다.
목록이 비어 있으면 일반 공동 수는 0개다. 선택한 공동은 각각 계획에 한 번 이상
포함되며, NPC 수 때문에 공동 공간이 더 필요할 때도 이 목록 안의 공동만 사용한다.
