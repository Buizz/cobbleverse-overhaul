...
### 영역·면 빠르게 채우기 (Effortless Building)

건축 팩에는 Effortless Building이 포함된다. 블록을 손에 든 상태에서 `Alt`를 길게 눌러
원형 메뉴를 열고 건축 모드를 고른다. 시작 위치를 우클릭한 뒤 끝 위치를 바라보고 다시
우클릭하면 미리 보인 영역에 블록을 놓는다.

| 모드 | 용도 |
|---|---|
| `Floor` | 같은 높이의 직사각형 면 채우기(바닥·천장) |
| `Wall` | 세로 직사각형 면 채우기 |
| `Cube` | 직육면체를 채우거나 외곽만 만들기 |
| `Line` | 기둥·보·테두리 만들기 |

`Ctrl+Z`는 되돌리기, `Ctrl+Y`는 다시 실행이다. 키가 다른 모드나 운영체제 단축키와
겹치면 Minecraft의 `설정 > 조작키`에서 Effortless Building 키를 변경한다. 이 기능으로
놓은 블록도 일반 블록이므로 빌더의 NBT 저장 범위 안에 있으면 그대로 내보내진다.
...
표시된다. 화면 상단에는 단축키와 막대기 조작 안내가 표시되며, `V`를 누르면 마우스로 누를 수 있는
공간 이동·크기 변경 버튼이 나온다. Effortless Building의 크기·배치 안내를 위해 화면 하단
중앙은 비워 둔다.
...
있습니다. 이 값은 구조물 종류와 관계없이 해당 NBT에만 적용됩니다.

- `0`: NBT의 Y=0을 계산된 지면 높이에 배치
- `1`: 한 칸 위에 배치
- `-1`: 한 칸 아래에 배치

의자나 가로등처럼 특정 종류에 자동으로 높이를 더하는 규칙은 없습니다. 값을 저장한 뒤
게임 모드 또는 개발 팩을 다시 빌드해야 새 월드 생성에 반영됩니다. 이미 생성이 끝난 마을은
자동 재배치되지 않으므로 새 월드에서 확인하거나 해당 마을을 다시 생성해야 합니다.
